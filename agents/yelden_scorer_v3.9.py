...
Changelog vs v1.3:
  + S_raw expandido de 5 → 11 componentes
  + Sortino com Confidence Adjustment (Lo 2002)
  + Expectancy normalizada (tecto 20%/trade)
  + Volatility Control (tecto 1.0%/dia)
  + Stability² com expoente 2.0 (Bailey & López de Prado)
  + Smoothness² com expoente 2.0
  + Drawdown: penalidade directa (não normalizado)
  + EMA acumulador (α=0.85, EMA₀=300)
  + SF: Stability Factor — memória de catástrofe 6 rodadas
  + Score final: 0–1000 (não 0–100)
  + Tectos recalibrados: Sharpe 2.5, PF 2.5, AvgR 1.2, Expectancy 20%
"""
...
CAP_EXPECTANCY    = 20.0         # Expectancy % máxima por trade
CAP_VOL           = 1.0          # Volatilidade diária máxima aceitável (%)
MIN_DOWNSIDE_DAYS = 20           # Mínimo de dias negativos para Sortino confiável
...
# Confidence: √(downside_trades/20) para consistência com CF trade-level
# Mas usamos ratio linear como em v4 (mais conservador)
sortino_conf   = min(n_neg_days / MIN_DOWNSIDE_DAYS, 1.0)
...
S_expectancy = min(max(expectancy_pct, 0) / CAP_EXPECTANCY, 1.0) * 100
...
if abs(mean_daily_pct) > 0:
    vol_to_return  = std_daily_pct / abs(mean_daily_pct)
    smoothness_raw = max(0.0, 1.0 - min(vol_to_return, 2.0) / 2.0)
else:
    smoothness_raw = 0.0
...
print(f"\n{SEP}")
print("  YELDEN PROTOCOL — AGENT SCORER v4.0")
print("  Multi-Factor S_raw (11 comp) × EMA × CF × SF  |  Escala 0–1000")
print(f"{SEP}")
...
print(f"  Expectancy (%/trade):        {expectancy_pct:.3f}%   [tecto={CAP_EXPECTANCY}%]")
...
print(f"  Sortino confidence:          {sortino_conf:.2f}  ({n_neg_days}/{MIN_DOWNSIDE_DAYS} dias negativos)")
...
print(f"  Smoothness (0→1):            {smoothness_raw:.4f}")
print(f"  Smoothness² score:           {S_smoothness:.1f}/100")
print(f"  Vol/Return ratio:            {std_daily_pct/abs(mean_daily_pct) if abs(mean_daily_pct)>0 else 'N/A':.3f}")
